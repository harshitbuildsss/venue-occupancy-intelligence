...
export default function ScrollHeroVideo() {
  ...
  const [showClickPrompt, setShowClickPrompt] = useState(false); // Controls the delayed text
  const isAnimating = useRef(false);

  const mallVideoRef = useRef(null);
  const gymVideoRef = useRef(null);

  // Sync video decoders strictly to active slides
  useEffect(() => {
    if (slide === 0) {
  ...
  useEffect(() => {
    let timer;
    if (slide === 2) {
      // Start a 3-second timer when we reach the image slide
      timer = setTimeout(() => {
        setShowClickPrompt(true);
      }, 3000);
    } else {
      // Reset immediately if we leave the slide
  ...
  return (
    <div id="scroll-hero-wrapper" className="fixed inset-0 z-50 w-full h-full overflow-hidden select-none bg-[#F3F2EC]">
      ...
      <div
        ...
        style={{
          transform: slide >= 3 ? 'translateY(-105%) scale(0.96)' : 'translateY(0%) scale(1)',
          borderRadius: slide >= 3 ? '3rem' : '0rem',
          opacity: slide < 2 ? 0 : 1,
          pointerEvents: slide === 2 ? 'auto' : 'none',
        }}
      >
        {/* Blinking Call to Action (Fades in after 3 seconds) */}
        <div 
          className={`absolute top-[6%] sm:top-[4%] z-20 pointer-events-none transition-opacity duration-1000 ${showClickPrompt ? 'opacity-100' : 'opacity-0'}`}
        >
        ...
        <div className="relative z-10 max-w-5xl px-6 text-center space-y-7 pointer-events-none">
          ...
          <div className="space-y-3">
            <h1 className="text-5xl sm:text-7xl font-serif font-medium text-white tracking-tight leading-[1.12] drop-shadow-[0_4px_16px_rgba(0,0,0,0.6)]">
              ...
              <span className="italic font-light text-white/95">before you even stepped out?</span>
            </h1>
          </div>
