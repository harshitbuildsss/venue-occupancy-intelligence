    ...
    <div className="rounded-2xl border border-slate-800 bg-slate-900/50 p-5 backdrop-blur-sm">
      <div className="flex items-center justify-between border-b border-slate-800 pb-4">
        <div className="flex items-center space-x-2.5">
          ...
          <div>
            <h3 className="text-sm font-bold text-white">Live Simulator Controller</h3>
            <p className="text-xs text-slate-400">
              Active target: <span className="font-semibold text-blue-400">{selectedVenue?.name || 'Select a venue'}</span>
            </p>
          </div>
        </div>
        ...
        <button
          ...
          className={`inline-flex items-center px-4 py-2 rounded-xl text-xs font-bold transition-all shadow-lg ${
            isSimulating
        ...
        <div className="flex flex-col justify-between">
          <div>
            ...
            <p className="text-xs text-slate-400 leading-relaxed mb-3">
              Trigger sudden traffic surges (e.g., arrival of a tour bus or movie release) to verify responsive UI alerts.
            </p>
          </div>
          <div className="flex space-x-2">
            <button
              ...
              disabled={!selectedVenue}
              className="flex-1 inline-flex items-center justify-center px-3 py-2 rounded-xl text-xs font-bold bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 transition-colors"
            >
              <Zap className="h-3.5 w-3.5 mr-1.5 text-emerald-400" /> Influx Burst (+6)
            </button>
            <button
              onClick={() => onInjectBurst('EXIT')}
              disabled={!selectedVenue || selectedVenue.currentOccupancy <= 5}
              className={`flex-1 inline-flex items-center justify-center px-3 py-2 rounded-xl text-xs font-bold border transition-colors ${
                !selectedVenue || selectedVenue.currentOccupancy <= 5
